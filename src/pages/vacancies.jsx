...
function Job({job, position, level, location}) {
    return (
        <div className="border-t-2 border-border_dark py-3 laptop:shadow-lg laptop:bg-light_grey laptop:mb-5 laptop:rounded-lg laptop:border-none laptop:px-8 laptop:py-7">
            ...
            <p className='text-body-sm font-normal text-dark_primary mt-2 laptop:text-body-md'>{position}</p>
            <p className='text-body-sm font-normal text-dark_primary laptop:text-body-md'>{level}</p>
            <p className='text-body-sm font-normal text-dark_primary laptop:text-body-md'>{location}</p>
        </div>  
    )
}
    ...
    return (
        <div>
             <div className='laptop:relative'>
                <img className='w-full object-cover laptop:max-h-[500px]' src={vacanciesImg} alt="vaccancies" />
             ...
             <div className='laptop:mx-44 laptop:flex'>
                <div className="mobile:hidden laptop:block laptop:w-4/12 laptop:mt-10">
                    ...
                    {
                        departments.map(department => (<CheckBox key={department.id} title={department.name}/>))
                    }
                    <h3 className='ml-10 text-body-lg font-bold mb-10 mt-20'>Level</h3>
                    {
                        levels.map(level => (<CheckBox key={level.id} title={level.name}/>))
                    }
                    <h3 className='ml-10 text-body-lg font-bold mb-10 mt-20'>Location</h3>
                    {
                        loacations.map(location => (<CheckBox key={location.id} title={location.name}/>))
                    }
                </div>
                <div className="mobile:mx-6 border-b-2 border-border_dark mt-10 mb-20 laptop:w-8/12 laptop:border-none">
                    <h3 className='text-center text-body-lg font-bold mb-5 laptop:mb-10'>Available Positions</h3>
                    {
                        jobs.map(job => (
            ...
            <div className="mx-6 laptop:mx-44">
                <h3 className='text-center text-body-lg font-bold mt-10 mb-5 laptop:mb-10'>Our Hiring Process</h3>
                <div className="mb-20 laptop:flex gap-10">
                    <div className="flex justify-center items-center bg-border_grey p-5 min-h-[120px] rounded-2xl relative mb-4 shadow-xl laptop:w-3/12 laptop:min-h-[200px]">
            ...
            {/* Hiring */}
            <div className="bg-light_grey px-6 py-8 laptop:py-16">
